...
**프롬프트 캐싱.** Bedrock 프롬프트 캐싱은 Claude 기준 캐시 가능 프리픽스 최소치가 ~1024 토큰. 효과가 있는 곳에만 적용:
- **에이전트(적용):** ~1.7K 토큰 시스템 프롬프트 + 도구 스키마가 매 ReAct 스텝마다, 그리고 멀티턴 세션 내내
...
- **파이프라인(미적용):** 단발성 프롬프트(랭커/다이제스트/트렌드/시각화 시놉시스, 모두 ≤~530 토큰이며 실행당
