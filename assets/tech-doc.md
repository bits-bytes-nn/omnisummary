...
**프롬프트 캐싱.** Bedrock 프롬프트 캐싱은 Claude 기준 캐시 가능 프리픽스 최소치가 약 1024 토큰입니다.
효과가 있는 곳에만 적용했습니다: 후속 **에이전트**는 약 1.7K 토큰 시스템 프롬프트 + 도구 스키마가 매 ReAct
...
시각화 시놉시스, 모두 약 530 토큰이며 실행당 1회 호출)는 캐시 최소치 미만이고 호출 간 재사용도 없어
